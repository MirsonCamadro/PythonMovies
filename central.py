...
def cargar_datos(lineas_archivo):
    ...
    for linea in lineas_archivo:
        titulo, popularidad, voto_promedio, cantidad_votos, generos_str = linea.split(',')
        ...
        pelicula_info = (titulo, float(popularidad), float(voto_promedio), int(cantidad_votos), generos)
...
def filtrar_y_ordenar(genero_pelicula):
    ...
    peliculas_filtradas = []
    for pelicula in info_peliculas:
        if genero_pelicula in pelicula[4]:
            peliculas_filtradas.append(pelicula[0])
...
def obtener_estadisticas(genero_pelicula, criterio):
    ...
    criterios = {"popularidad": 1, "voto promedio": 2, "cantidad votos": 3}
    valores = [
        pelicula[criterios[criterio]] for pelicula in info_peliculas if genero_pelicula in pelicula[4]
    ]
    ...
    if valores:
        ...
        return max_valor, min_valor, promedio
